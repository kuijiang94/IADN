...
img_path = '.\\test\\test_data\\deraing\\TEST1200\\inputcrop\\'
file = os.listdir(img_path)
save_path = '.\\test\\test_data\\deraing\\IADN_40\\'
...
for f in file:
    ...
    if num==1:
        ...
        model = Model(x_rain, is_training, 1, spt)#16
        init = tf.global_variables_initializer()
        sess = tf.Session()
        sess.run(init)
        saver = tf.train.Saver()
        saver.restore(sess, '../IADN/epoch40')#93
        vars_all=tf.trainable_variables()
        print ('Params:',np.sum([np.prod(v.get_shape().as_list()) for v in vars_all]))
        st_time=time.time()#rain_res,imitation
        fake = sess.run(
            [model.imitation],
